...
后文将实验者路由器称为被测路由器。进行后文测试时，同组的被测路由器应当使用相同的硬件设计和软件设计，仅允许 IP 地址和/或 MAC 地址配置有所不同。同时，为加快路由收敛，被测路由器 RIP 路由协议的定时器参数选取为：更新定时器为 3 秒，超时定时器为 18 秒，垃圾回收定时器为 12 秒，与 RFC 文档规定不同。

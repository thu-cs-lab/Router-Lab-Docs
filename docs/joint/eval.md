...
* 连通性：首先在 PC0 上运行 `ping -c 5 10.1.2.2` ，然后运行 `sudo ping -c 1000 -i 0.01 10.1.2.2` 。然后在 PC0 上运行 `ping -c 5 10.2.2.2` ，接着运行 `sudo ping -c 1000 -i 0.01 10.2.2.2` 。最后在 PC1 上运行 `ping -c 5 10.2.2.2` ，接着运行 `sudo ping -c 1000 -i 0.01 10.2.2.2` 。若上述三种情况后一命令丢包率均不超过 1‰ 则认定被测路由器在该拓扑下连通。测试时注意观察 hop limit（下同）。

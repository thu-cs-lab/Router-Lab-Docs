...
6. 导出 R1 和 R3 上 BIRD 的路由表，和答案进行比对。
7. 在 PC2 上运行 `iperf3 -s`，在 PC1 上运行 `iperf -c 192.168.5.1 -O 5 -P 10`，按照 Bitrate 给出分数，测试转发的效率。
...
7. 在 PC2 上运行 `iperf3 -s`，在 PC1 上运行 `iperf -c 192.168.5.1 -O 5 -P 10`，按照 Bitrate 给出分数，测试转发的效率。
8. 在 PC2 上运行 `iperf3 -s`，在 PC1 上运行 `iperf -c 192.168.5.1 -u -l 16 -t 5 -b 1G -O 5`，按照接收方实际收到的 Datagram 数量（即 Total Datagram 减去 Lost）给出分数，测试转发的效率。

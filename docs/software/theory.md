...
为了让路由器找到正确的路由表，<!-- 本实验要实现的就是 -->需要一个简单的路由协议：RIP 协议。简单来说，假如现在要搭建这样一个快递网络：
...
在本次实验中，<!-- 你可以在四种协议里进行选择一种来实现，其中 -->你需要实现的是 OSPF 路由协议。OSPF 和 RIPng 都是路由协议，二者的区别在于，RIPng 实现的是距离向量（矢量）协议，而 OSPF 实现的是链路状态协议。通过完成这个实验，就是希望同学们用代码把课上讲授的路由协议实现出来，从而获得对路由协议更加深刻的理解。

对于 RIPng 协议，它实现的是距离向量协议，因此就需要维护距离向量，定期发送给邻居的路由器；另一方面，邻居收到距离向量以后，更新自己的距离向量，从而得到新的路由表。

<!-- 1. 从路由表中获取距离向量，把距离向量封装成 RIPng Response，发送给所有邻居
2. 收到 RIPng Response 的时候，使用距离向量，更新自己的路由表 -->

而对于 OSPF 协议，它实现的是链路状态协议。链路状态协议工作的时候，需要首先学习出网络的拓扑，再对网络拓扑执行最短路算法，得到的结果就可以转化为路由表。具体到代码，需要实现：

1. 从链路状态信息，构造出网络拓扑的内部表示
2. 计算最短路径，使用最短路径，更新自己的路由表

从核心算法来说，链路状态协议要更加复杂，因此在本实验中，为了平衡难度，代码框架提供了 OSPF 的大部分功能，只需要你实现核心的算法。

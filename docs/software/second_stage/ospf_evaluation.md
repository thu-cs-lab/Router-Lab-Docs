...
??? example "可供参考的例子"
    ...
    ```
    No.1:       R1 从 r1r2 接口向 AllSPFRouters 组播地址，即 ff02::5，发送 OSPF Hello 报文。
    No.2~No.3:  MLD（Multicast Listener Discovery）协议相关报文，用于加入组播组，本实验中不用关心。
    No.4:       R2 路由器启动了，于是开始发送 OSPF Hello 报文。R1 第一次收到 R2 发来的 OSPF Hello 报文，
                R1 开始维护关于 R2 的邻居状态。
    ...
    No.12:      R1 收到 R2 的 DD 报文，由于 R2 的 Router ID 比自己大，于是将自己设为 Follower（Slave），
                ...
                其中 Init 位和 Leader（Master）位设置为 0，同时携带自己的 LSA 头部。
    No.13:      R2 收到 R1 的 DD 报文，将自己设为 Leader（Master），邻居状态变为 Exchange，将自己的
                DD Sequence Number 增加一，Leader（Master）位设置为 1，同时携带自己的 LSA 头部发送。
    No.14:      R1 收到 R2 的 DD 报文，发现 More 位为 0，于是使用收到 DD 报文的 DD Sequence Number
                回复 R2 的 DD 报文（因为 R1 是 Follower），邻居状态变为 Loading。
    No.15:      R1 发送 LS Request，请求自己没有的 R2 的两个 LSA。
    No.16:      R2 收到 R1 的 DD 报文（No.14），发现 More 位为 0，于是邻居状态变为 Loading，
                然后 R2 发送 LS Request，请求自己没有的 R1 的两个 LSA。
    No.17:      R1 发送 LS Update 回复 No.16 的 LS Request。R2 收到此报文后，便完成了所有 LSA 的请求，
                维护的 R1 邻居状态变为 Full。
    ...
    No.24:      由于 R1 收到过 R2 发来的报文，故邻居缓存中有 R2 地址对应的 MAC 地址，此处是 R1
                正在维护邻居缓存有效性，进行 NUD 检测。
    No.25:      R2 对 No.24 NUD 检测的回复。
    No.26:      R1 维护的 R2 邻居状态变为 Full 后，自身的 Router LSA 中添加了到 R2 的连接信息，
                向各个邻居洪泛这个 LSA 的更新，此处的延迟发送与 BIRD 的实现相关。
    No.27:      R2 在间隔 5 秒后，再一次发送 OSPF Hello 报文。
    ...
    No.32:      R2 维护的 R3 邻居状态变为 Full 后，自身的 Router LSA 中添加了到 R3 的连接信息，
                向各个邻居洪泛这个 LSA 的更新。
    No.33:      R2 收到 R3 的 Intra-Area-Prefix LSA，洪泛 LS Update 给 R1。
    No.34:      R1 在间隔 5 秒后，再一次发送 OSPF Hello 报文。
    No.35:      R1 收到 R2 的 LS Update（No.31、No.32、No.33），回复 LS Acknowledge。此处为延迟发送。
    No.36:      R1 维护邻居缓存，进行 NUD 检测。
    No.37:      R2 向 R1 洪泛 R3 的 Router LSA 更新，这说明 R3 维护的 R2 邻居状态已变为 Full，
                并且 R3 已经向 R2 洪泛了更新。
    No.38:      R2 回复 No.36 的 NUD 检测。
    No.39:      R1 收到 R2 的 LS Update（No.37），回复 LS Acknowledge。
    No.40~NO.45: R1 和 R2 继续每隔 5 秒发送 OSPF Hello 报文，维持邻居状态。
    ...
    No.48~No.54: PC1 ping PC2 的剩余三个来回。
    No.55~No.64: PC2 ping PC1 的四个来回。
    ```

    `ospf-r3.pcap` 的抓包结果是类似的。

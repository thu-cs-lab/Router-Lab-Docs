...
第三部分是针对多人的互联测试，每次测试采用五位同学的代码，分别运行在五个网络设备上。为了完成互联评测，你需要使用 master 分支上的同一份代码完成至少两次评测，并在 TANLabs 上标记两个最终提交。评测中其余四名同学不能出现重复，即共挑选八名同学，将你的代码分别与前四名和后四名同学联网进行评测。

...
2026-2027 年秋季《计算机网络原理课程》的实验包含以下内容：

- 必做：软件路由器实验，内有两个阶段：
    1. 第一阶段：编程作业
    2. 第二阶段：真机实验，在以下两个选项中选择一个完成：
        1. OSPF 路由器（推荐）
        2. RIPng 路由器
        <!-- 3. DHCPv6 路由器 -->
        <!-- 4. TFTP 客户端和服务端 -->
    <!-- 3. 第三阶段：组网实验：将多名同学在个人实验的实现的软件，放在同一个网络中进行测试 -->
- 进阶选做：P4 交换机实验（学期初报名选拔）
...
- 2025-2026 学年：软件实验删除了 TFTP 和 DHCPv6 的实验选项，删除了组网互联实验，原本的硬件路由器实验停开，新增 P4 交换机实验

## 项目成员

* 框架设计：张宇翔（@z4yx）
* 代码与文档编写：陈晟祺（@Harry-Chen）、陈嘉杰（@jiegec）
* 硬件实验板设计：高一川（@gaoyichuan）
...
* （2026-2027 学年秋季学期）尚子智、彭汉一、张皓晨、谢语桐、马宇辰、陈毓椿
